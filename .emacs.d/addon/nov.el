...
Sets `header-line-format' according to `nov-header-line-format'."
  (progn
    ...
    (if (null header-line-format)
        (if (null header-line-format-stashed)
        ;; do when header line is nil (restore)
            (progn
              ;; create header-line-format if not hidden already
      ...
      (progn
        ...
        (setq-local header-line-format-stashed
	                (and nov-header-line-format
	                 (let ((title (cdr (assq 'title nov-metadata)))
		                   (chapter-title (car (dom-children dom))))
	                   (when (not chapter-title)
...
;; how many extra scrollup/downs are necessary to change chapter
(setq nov-scroll-up-buffer-zone 7)
(setq nov-scroll-down-buffer-zone 3)
...
(defun nov-scroll-up (arg)
  ...
  ;;    (scroll-up arg)))
  (progn
    (setq-local nov-scroll-down-counter nov-scroll-down-buffer-zone)
    (if (>= (window-end) (point-max))
        (progn
          (if (boundp 'nov-scroll-up-counter)
              () ;; do nothing if already defined
            ;; else, define
            ;; reset scroll down counter to nov-scroll-up-buffer-zone
            (setq nov-scroll-up-counter nov-scroll-up-buffer-zone))
          (if (= nov-scroll-up-counter 0)
              (progn
                (nov-next-document)
                (setq-local nov-scroll-up-counter nov-scroll-up-buffer-zone))
            (progn
              (setq-local nov-scroll-up-counter (- nov-scroll-up-counter 1))
              ;; overscroll:
              (scroll-up arg))))
              ;;)))
      (scroll-up arg))))
...
(defun nov-scroll-down (arg)
  ...
  (interactive "P")
  (progn
    ;; reset scroll up counter to nov-scroll-down-buffer-zone
    (setq-local nov-scroll-up-counter nov-scroll-up-buffer-zone)
    (if (and (<= (window-start) (point-min))
             (> nov-documents-index 0))
        (progn
          (progn
            (if (boundp 'nov-scroll-down-counter)
                ()
              (setq nov-scroll-down-counter nov-scroll-down-buffer-zone)) ;; make sure it's defined
            (if (= nov-scroll-down-counter 0)
                (progn
                  (nov-previous-document)
                  (goto-char (point-max))
                  (setq-local nov-scroll-down-counter nov-scroll-down-buffer-zone)
                  ;; make it take same number of bumps down to return if we just changed chapter
                  (setq-local nov-scroll-up-counter nov-scroll-down-buffer-zone))
              (progn
                (setq-local nov-scroll-down-counter (- nov-scroll-down-counter 1))
                ;; overscroll:
                ;;(scroll-down arg)))))
                ))))
      (scroll-down arg))))
